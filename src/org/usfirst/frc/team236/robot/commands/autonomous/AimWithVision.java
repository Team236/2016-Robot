...
public class AimWithVision extends Command {
    ...
    private final double CameraPivotHeight = 0.3; // Height of camera above
						  // ground in meters
    private final double CameraArmLength = 0.7; // Length of camera arm
    private final double TargetHeight = 8.0 * 0.3048; // Height of center of
						      // target in meters
    ...
    // Constants
    // How close we have to get (in degrees) to targetHeading before we're
    // satisfied
    // Depending on how fast the robot turns, this number may need to be
    // increased to get it to stop turning;
    // however, the larger the number, the greater the error. We want the
    // smallest number we can get where
    ...
    // Increasing this number will slow the robot down; decreasing it will speed
    // the robot up.
    ...
    // How close we have to get (in degrees) to targetHeading before we're
    // satisfied
    // Depending on how fast the arm moves, this number may need to be increased
